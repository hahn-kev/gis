...
!(function () {
  ...
  var require = (function () {
    ...
    require.define = function (m) {
      modules[m] = module.exports;
      module.exports = {};
    };
    return require;
  })();
...
// - use arrays in CompositeEventStream to avoid nested debouncings
  module.exports = (function (window, document) {
    ...
    function TimeoutEventStream(options) {
      ...
      // flag that says whether the observer is still needed or not
      var rid = 0;
      var timeout = (typeof(options) == "number") ? (+options) : ("timeout" in options ? +options.timeout : 333);
    ...
    function MouseEventStream() {
      var self = this;
      var pointermove = (("PointerEvent" in window) ? "pointermove" : (("MSPointerEvent" in window) ? "MSPointerMove" : "mousemove"));
  ...
  module.exports = (function (window, document) {
    ...
    var cssCascade = {
      ...
      polyfillStyleInterface: function (cssPropertyName) {
        ...
        var styleProtos = [];
        try {
          styleProtos.push(Object.getPrototypeOf(document.documentElement.style) || CSSStyleDeclaration);
        } catch (ex) {
        }
...
////////////////////////////////////////
  ...
  module.exports = (function (window, document) {
    ...
    VirtualStylesheet.prototype.enable = function () {
      this.factory.enableAllRules(this.name);
      this.enabled = true;
    }

    VirtualStylesheet.prototype.disable = function () {
      this.factory.disableAllRules(this.name);
      this.enabled = false;
    }
...
//
  module.exports = (function (window, document) {
    ...
    cssUnits.convertToPixels.converters = {
      ...
      'cm': function (x) {
        return x / 0.02645833333;
      },
      'mm': function (x) {
        return x / 0.26458333333;
      },
      'pt': function (x) {
        return x / 0.75;
      },
      ...
      'em': function (x, e) {
        return x * parseFloat(e ? getComputedStyle(e).fontSize : 16);
      },
      'rem': function (x, e) {
        return x * parseFloat(e ? getComputedStyle(e.ownerDocument.documentElement).fontSize : 16);
      },
      'vw': function (x, e) {
  ...
  module.exports = (function (window, document) {
    ...
    GridTrackBreadth.prototype = {
      toString: function () {
        if (this.minType == this.maxType && this.minValue == this.maxValue) {
          switch (this.minType) {
            case TRACK_BREADTH_AUTO:
              return "auto";
            case TRACK_BREADTH_LENGTH:
              return this.minValue + "px";
            case TRACK_BREADTH_FRACTION:
              return this.minValue + "fr";
            case TRACK_BREADTH_PERCENTAGE:
              return this.minValue + "%";
            case TRACK_BREADTH_MIN_CONTENT:
              return "min-content";
            case TRACK_BREADTH_MAX_CONTENT:
              return "max-content";
          }
        } else {
          var min = "auto";
          ...
          switch (this.minType) {
            case TRACK_BREADTH_AUTO:
              ...
              break;
            case TRACK_BREADTH_LENGTH:
              min = this.minValue + "px";
              break;
            case TRACK_BREADTH_FRACTION:
              min = this.minValue + "fr";
              break;
            case TRACK_BREADTH_PERCENTAGE:
              min = this.minValue + "%";
              break;
          ...
          switch (this.maxType) {
            case TRACK_BREADTH_AUTO:
              ...
              break;
            case TRACK_BREADTH_LENGTH:
              max = this.maxValue + "px";
              break;
            case TRACK_BREADTH_FRACTION:
              max = this.maxValue + "fr";
              break;
            case TRACK_BREADTH_PERCENTAGE:
              max = this.maxValue + "%";
              break;
    ...
    GridItem.prototype = {
      ...
      parseLocationInstructions: function (specifiedStart, specifiedEnd, cssText) {
        ...
        var value = unfiltred_value.filter(function (o) {
          return !(o instanceof cssSyntax.WhitespaceToken);
        });
        value.toCSSString = function () {
          return unfiltred_value.toCSSString();
        }
        var I = 0;
        ...
        if (!value[I]) {
          console.error("INVALID DECLARATION: grid-column/row: " + value.toCSSString() + " (empty declaration)");
          this.buggy = true;
          return;
        }
        ...
        gridColumnStart: while (true) {
          if (value[I] instanceof cssSyntax.IdentifierToken) {
            ...
            if (value[I].value == "span") {
              ...
              if (!value[++I]) {
                console.error("INVALID DECLARATION: grid-column/row: " + value.toCSSString() + " (span is not a valid line name, more tokens expected)");
                this.buggy = true;
                return;
              }
        ...
        if (specifiedEnd.type == LOCATE_SPAN && specifiedStart.type == LOCATE_SPAN) {
          specifiedEnd.type = LOCATE_AUTO;
          specifiedEnd.index = undefined;
          specifiedEnd.name = undefined;
        }
    ...
    GridLayout.prototype = {
      ...
      updateFromElement: function () {
        ...
        var sortableItems = this.items.map(function (item, i) {
          return {item: item, order: item.order, position: i}
        });
        ...
        this.items = sortableItems.map(function (data) {
          return data.item;
        });
        ...
        var style = usedStyleOf(this.element);
        var cssText = '';
        if (cssText = style["grid-template"]) {
          this.parseGridTemplate(cssText);
        }
        if (cssText = style["grid-template-rows"]) {
          this.parseRowsTemplate(cssText);
        }
        if (cssText = style["grid-template-columns"]) {
          this.parseColumnsTemplate(cssText);
        }
        if (cssText = style["grid-template-areas"]) {
          this.parseAreasTemplate(cssText);
        }
        if (cssText = style["grid-auto-rows"]) {
          this.parseAutoRowsBreadth(cssText);
        }
        if (cssText = style["grid-auto-columns"]) {
          this.parseAutoColumnsBreadth(cssText);
        }
      ...
      parseTrackBreadth: function (value, I) {
        ...
        if (value[I] instanceof cssSyntax.Func && value[I].name == "minmax") {
          ...
          value = args[0].filter(function (t) {
            return !(t instanceof cssSyntax.WhitespaceToken)
          });
          I = 0;
          ...
          value = args[1].filter(function (t) {
            return !(t instanceof cssSyntax.WhitespaceToken)
          });
          I = 0;
          var data = parseTrackBreadthToken.call(this);
      ...
      parseAutoRowsBreadth: function (cssText) {
        ...
        if (data.result) {
          this.defaultYSize = data.result;
        } else {
          throw "TODO: better error message";
        }
        return;
      ...
      parseGridTemplate: function (cssText) { // TODO: I used some lazy heuristics here
        ...
        if (cssTextSections.length == 2) {
          if (this.parseColumnsTemplate(cssTextSections[0])) {
            return buggy = true;
          }
        ...
        if (/"|'/.test(cssText)) {
          ...
          var strings = [];
          cssText = cssText.replace(/\s*("(?:.*?)"|'(?:.*?)')\s*([-_a-zA-Z0-9]*)\s*/g, function (data, str, size) {
            strings.push(str);
            return ' ' + (size || "auto") + ' ';
          });
          ...
          // parse rows now
          if (this.parseRowsTemplate(cssText)) {
            return buggy = true;
          }

          // parse areas now
          if (this.parseAreasTemplate(strings.join(' '))) {
            return buggy = true;
          }
      ...
      parseAreasTemplate: function (cssText) {
        ...
        while (value[I]) {
          ...
          while (str !== '') {
            ...
            // extract next token
            var data = regexp.exec(str);
            if (!data || data.length != 2) {
              return buggy = true;
            }
            str = str.substr(data[0].length);
            var cell = data[1];

            // update cell max pos (ignore empty cells)
            if (cell != '.' && cell[0] != '.') {
              if (!areas[cell]) {
                areas[cell] = {xStart: columns.length, xEnd: columns.length + 1, yStart: I - 1, yEnd: I};
              }
              if (areas[cell].xStart > columns.length) {
                return buggy = true;
              }
              if (areas[cell].yStart > I - 1) {
                return buggy = true;
              }
      ...
      autoGrow: function () {
        ...
        for (var i = this.items.length; i--;) {
          ...
          if (item.xEnd > 0) {
            growX.call(this, item.xEnd);
          }
          if (item.yEnd > 0) {
            growY.call(this, item.yEnd);
          }
          if (item.xEnd > 0 && item.yEnd > 0) {
            continue;
          }
        ...
        if (this.growY) {
          ...
          for (var i = 0; i < this.items.length; i++) {
            ...
            if (item.specifiedXStart.type == LOCATE_LINE) {
              ...
              var nextStep = function () {
                cursor.x++;
                if (cursor.x + spanX > this.rcMatrix[0].length) {
                  cursor.y++;
                  this.ensureRows(cursor.y + spanY);
                  cursor.x = 0;
                }
          ...
          for (var i = 0; i < this.items.length; i++) {
            ...
            if (item.specifiedYStart.type == LOCATE_LINE) {
              ...
              var nextStep = function () {
                cursor.y++;
                if (cursor.y + spanY > this.rcMatrix[0].length) {
                  cursor.x++;
                  this.ensureRows(cursor.x + spanX);
                  cursor.y = 0;
                }
                return true;
              }
      ...
      computeAbsoluteTrackBreadths: function () {
        ...
        var computeTrackBreadth = function (xSizes, specifiedSizes, getMinWidthOf, getMaxWidthOf, getXStartOf, getXEndOf) {
          ...
          for (var x = specifiedSizes.length; x--;) {
            ...
            if (specifiedSizes[x].minType == TRACK_BREADTH_MIN_CONTENT || specifiedSizes[x].minType == TRACK_BREADTH_AUTO) {
              ...
              for (var i = this.items.length; i--;) {
                var item = this.items[i];
                var item_xStart = getXStartOf(item);
                var item_xEnd = getXEndOf(item);
            ...
            // If the track has a max-content min track sizing function
              ...
              for (var i = this.items.length; i--;) {
                ...
                xSizes[x].base = Math.max(xSizes[x].base, getMaxWidthOf(item));
                items_done++;
                dontCountMaxItems = true;
            ...
            if (specifiedSizes[x].maxType == TRACK_BREADTH_MIN_CONTENT) {
              ...
              for (var i = this.items.length; i--;) {
                ...
                if (xSizes[x].limit == infinity) {
                  ...
                  xSizes[x].limit = Math.max(xSizes[x].limit, getMinWidthOf(item));
                }

                if (!dontCountMaxItems) {
                  items_done++;
                }
            ...
            // If the track has a max-content max track sizing function
              ...
              for (var i = this.items.length; i--;) {
                ...
                if (xSizes[x].limit == infinity) {
                  xSizes[x].limit = getMaxWidthOf(item);
                }
                else {
                  xSizes[x].limit = Math.max(xSizes[x].limit, getMaxWidthOf(item));
                }
            ...
            ItemLoop: for (var i = this.items.length; i--;) {
              ...
              var distributeFreeSpace = function (requiredSpace, kind /*'base'|'limit'*/, target /*'min-content'|'max-content'*/) {
                ...
                while (true) {
                  ...
                  rows_and_limits.sort(function (a, b) {
                  ...
                  if (kind == 'base') {
                    ...
                    var tracks = rows_and_limits.filter(function (b) {
        ...
        var computeTrackBreadthIncrease = function (xSizes, specifiedSizes, fullSize, getMinWidthOf, getMaxWidthOf, getXStartOf, getXEndOf) {
          ...
          while (true) {
            ...
            if (spaceToDistribute <= 1 / 1024) {
              return;
            } // NOTE: the space may never become 0 due to a rounding issue

            // Distribute space up to growth limits
            var tracks = rows_and_limits = rows_and_limits.filter(function (b) {
              return ((b.minIsMinContent || b.minIsMaxContent) && b.base < b.limit);
            }, 0);
            var trackAmount = tracks.length;
            if (trackAmount <= 0) {
              return;
        ...
        var computeFlexibleTrackBreadth = function (xSizes, specifiedSizes, fullSize, getMinWidthOf, getMaxWidthOf, getXStartOf, getXEndOf) {
          ...
          if (fullSize == 0) {
            ...
            for (var i = this.items.length; i--;) {
              ...
              // gather some pieces of data about the tracks
              var spaceToDistribute = getMaxWidthOf(item);
              var flexFactorSum = 0;
              for (var cx = item_xStart; cx < item_xEnd; cx++) {
            ...
            while (tracks.length > 0) {
              ...
              var currentFraction = spaceToDistribute / fractionSum;
              var restart = false;
        ...
        var getMinWidthOf = function (item) {
          return item.minWidth + item.hMargins;
        };
        var getMaxWidthOf = function (item) {
          return item.maxWidth + item.hMargins;
        };
        var getXStartOf = function (item) {
          return item.xStart;
        };
        var getXEndOf = function (item) {
          return item.xEnd;
        };
        ...
        var getMinHeightOf = function (item) {
          return item.element.offsetHeight + item.vMargins;
        };
        var getMaxHeightOf = function (item) {
          return item.element.offsetHeight + item.vMargins;
        };
        var getYStartOf = function (item) {
          return item.yStart;
        };
        var getYEndOf = function (item) {
          return item.yEnd;
        };
      ...
      generatePolyfilledStyle: function () {
        ...
        if (["auto", "0px"].indexOf(usedStyle.getPropertyValue("width")) >= 0) {
          runtimeStyleData["width"] = grid_width + 'px';
        }
        if (["auto", "0px"].indexOf(usedStyle.getPropertyValue("height")) >= 0) {
          runtimeStyleData["height"] = grid_height + 'px';
        }
        runtimeStyle.set(this.element, runtimeStyleData);
      ...
      findXLine: function (name, startIndex, skipCount, dontFallback) {
        ...
        if (!dontFallback) {
          console.warn('[CSS-GRID] Missing ' + (skipCount + 1) + ' lines named "' + name + '" after line ' + startIndex + '.');
          last = this.xLines.length + skipCount + 1;
          this.ensureRows(last);
        }
        return last;
      ...
      findYLine: function (name, startIndex, skipCount, dontFallback) {
        ...
        if (!dontFallback) {
          console.warn('[CSS-GRID] Missing ' + (skipCount + 1) + ' lines named "' + name + '" after line ' + startIndex + '.');
          last = this.yLines.length + skipCount + 1;
          this.ensureColumns(last);
        }
        return last;
...
// TOOD: document the "no_ms_grid_implementation" flag?
  ...
  !(function (window, document) {
    ...
    if ("gridRow" in document.body.style) {
    ...
    var enablePolyfill = function () {
      ...
      for (var i = gridProperties.length; i--;) {
        cssCascade.polyfillStyleInterface(gridProperties[i]);
      }
      for (var i = gridItemProperties.length; i--;) {
        cssCascade.polyfillStyleInterface(gridItemProperties[i]);
      }
            ...
            if (element.gridLayout) {
              ...
              if ("MutationObserver" in window) {
                ...
                element.addEventListener('DOMSubtreeModified', function () {
                  if (!element.gridLayout.isLayoutScheduled) {
                    element.gridLayout.scheduleRelayout();
                  }
                }, true);
              }
              ...
              window.addEventListener('load', function () {
                element.gridLayout && element.gridLayout.scheduleRelayout()
              });

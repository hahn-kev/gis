...
export class JwtHelperService {
  ...
  private static b64DecodeUnicode(str: any) {
    return decodeURIComponent(Array.prototype.map.call(JwtHelperService.b64decode(str), (c: any) => {
  ...
  public static decodeToken(token: string): any {
    ...
    let decoded = JwtHelperService.urlBase64Decode(parts[1]);
    if (!decoded) {
  ...
  public static getTokenExpirationDate(token: string): Date {
    let decoded: any;
    decoded = JwtHelperService.decodeToken(token);
  ...
  public static isTokenExpired(token: string, offsetSeconds?: number): boolean {
    let date = JwtHelperService.getTokenExpirationDate(token);

...
namespace Backend.Linq2DbIdentity
{
        ...
        where TRoleClaim : IdentityRoleClaim<TKey>, new()
    {
        ...
        protected virtual async Task<IdentityResult> UpdateAsync(DataConnection db,
            ...
            CancellationToken cancellationToken)
        {
            var result = await Task.Run<int>(() => db.UpdateConcurrent<TRole, TKey>(role), cancellationToken);
        ...
        private async Task<IdentityResult> DeleteAsync(DataConnection db,
            TRole role,
            CancellationToken cancellationToken)
        {
            var result = await Task.Run<int>(() =>
                    Queryable.Where(db.GetTable<TRole>(),
                            _ => _.Id.Equals(role.Id) && _.ConcurrencyStamp == role.ConcurrencyStamp)
                        .Delete(),
                cancellationToken);
        ...
        protected virtual async Task<TRole> FindByIdAsync(DataConnection db,
            TKey roleId,
            CancellationToken cancellationToken)
        {
        ...
        protected virtual async Task<TRole> FindByNameAsync(DataConnection db,
        ...
        protected virtual async Task<IList<Claim>> GetClaimsAsync(DataConnection db,
            ...
            CancellationToken cancellationToken)
        {
            return await Queryable.Select(Queryable.Where(db.GetTable<TRoleClaim>(), rc => rc.RoleId.Equals(role.Id)),
                    c => c.ToClaim())
                .ToListAsync<Claim>(cancellationToken);
        }
        ...
        /// </param>
        /// <returns>The <see cref="Task" /> that represents the asynchronous operation.</returns>
        public async Task AddClaimAsync(TRole role,
            Claim claim,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ThrowIfDisposed();
        ...
        protected virtual async Task AddClaimAsync(DataConnection db,
            TRole role,
            Claim claim,
            CancellationToken cancellationToken)
        {
        ...
        /// </param>
        /// <returns>The <see cref="Task" /> that represents the asynchronous operation.</returns>
        public async Task RemoveClaimAsync(TRole role,
            Claim claim,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ThrowIfDisposed();
        ...
        protected virtual async Task RemoveClaimAsync(DataConnection db,
            TRole role,
            Claim claim,
            CancellationToken cancellationToken)
        {
            await Task.Run<int>(() =>
                    Queryable.Where(db.GetTable<TRoleClaim>(),
                            rc => rc.RoleId.Equals(role.Id) && rc.ClaimValue == claim.Value &&
